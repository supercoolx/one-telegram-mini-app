...
const vertexShader = `
    ...
    void main() {
        float noise = u_size * pnoise(position + u_time, vec3(10.0));
...
function WhiteSphereMesh({ EE }) {
    ...
    const handleClick = () => {
        ...
        for (let i = 0; i < positions.length; i += 3) {
            ...
            // const offset = 0.2 * Math.sin(2 * time + x * 0.5) + 0.3 * Math.cos(2 * time + y * 0.5);
            // if (x < max) max = x;
    ...
    return (
        <mesh onClick={handleClick} ref={meshRef}>
            ...
            <shaderMaterial ref={materialRef} uniforms={uniforms} wireframe={true} vertexShader={vertexShader} fragmentShader={fragmentShader} transparent={true} />
        </mesh>
    );
}

export default function WhiteSphere({ EE }) {

    return (
        <Canvas style={{ width: '100vw', height: '100vh' }}>
            <WhiteSphereMesh EE={EE} />
        </Canvas>
    );
}
